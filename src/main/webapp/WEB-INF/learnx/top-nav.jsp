<div class="container">
    <header class="d-flex flex-wrap align-items-center justify-content-center justify-content-md-between py-3 mb-4 border-bottom">
        ...
        <ul class="nav col-12 col-md-auto mb-2 justify-content-center mb-md-0">
            <li><a href="${appURL}/learnx" class="nav-link px-2<c:if test="${pageTitle eq 'Home'}">link-secondary text-decoration-underline</c:if>"">Home</a></li>
            ...
            <c:choose>
                <c:when test="${sessionScope.activeUser.privileges eq 'student'}">
                    <li><a href="student" class="nav-link px-2" <c:if test="${pageTitle eq 'Student Dashboard'}">link-secondary text-decoration-underline</c:if>">Student Dashboard</a></li>
                </c:when>
                <c:when test="${sessionScope.activeUser.privileges eq 'teacher'}">
                    <li><a href="teacher" class="nav-link px-2" <c:if test="${pageTitle eq 'Teacher Dashboard'}">link-secondary text-decoration-underline</c:if>">Teacher Dashboard</a></li>
                </c:when>
                <c:when test="${sessionScope.activeUser.privileges eq 'admin'}">
                    <li><a href="admin" class="nav-link px-2" <c:if test="${pageTitle eq 'Admin Dashboard'}">link-secondary text-decoration-underline</c:if>">Admin Dashboard</a></li>
                </c:when>
        ...
        <div class="col-md-3 text-end">
            <c:choose>
                ...
                <c:otherwise>
                    <a href="${appURL}/logout" class="btn btn-outline-orange me-2">Logout</a>
