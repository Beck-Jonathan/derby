        ...
        <ul class="nav col-12 col-md-auto mb-2 justify-content-center mb-md-0">
            ...
            <c:choose>
                <c:when test="${sessionScope.activeUser.privileges eq 'student'}">
                    <li><a href="student" class="nav-link px-2" <c:if test="${pageTitle eq 'Student Dashboard'}">link-secondary text-decoration-underline</c:if>"><fmt:message key="topNav.student"/></a></li>
                </c:when>
                <c:when test="${sessionScope.activeUser.privileges eq 'teacher'}">
                    <li><a href="teacher" class="nav-link px-2" <c:if test="${pageTitle eq 'Teacher Dashboard'}">link-secondary text-decoration-underline</c:if>"><fmt:message key="topNav.teacher"/></a></li>
                </c:when>
                <c:when test="${sessionScope.activeUser.privileges eq 'admin'}">
                    <li><a href="admin" class="nav-link px-2" <c:if test="${pageTitle eq 'Admin Dashboard'}">link-secondary text-decoration-underline</c:if>"><fmt:message key="topNav.admin"/></a></li>
                </c:when>
